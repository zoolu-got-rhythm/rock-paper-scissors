...
export enum ResultWinCondition {
    ROCK_BEATS_SCISSORS = "ROCK_BEATS_SCISSORS",
    PAPER_BEATS_ROCK = "PAPER_BEATS_ROCK",
    SCISSORS_BEATS_PAPER = "SCISSORS_BEATS_PAPER",
}
...
class Match extends Observable<Match, MatchEvents> {
    ...
    public getWinnerOfRound() {
        ...
        if (this.playerA.currentHand === Hand.ROCK && this.playerB.currentHand === Hand.SCISSORS) {
            // p-a rock > p-b scissors
            this.score[this.playerA.id] += 1;
            this.howWasRoundWon = ResultWinCondition.ROCK_BEATS_SCISSORS;
            this.notify(MatchEvents.PLAYER_A_WINS_ROUND);
        } else if (this.playerA.currentHand === Hand.ROCK && this.playerB.currentHand === Hand.PAPER) {
            // p-a rock < p-b paper
            this.score[this.playerB.id] += 1;
            this.howWasRoundWon = ResultWinCondition.PAPER_BEATS_ROCK;
            this.notify(MatchEvents.PLAYER_B_WINS_ROUND);
        } else if (this.playerA.currentHand === Hand.SCISSORS && this.playerB.currentHand === Hand.PAPER) {
            // p-a scissors > p-b paper
            this.score[this.playerA.id] += 1;
            this.howWasRoundWon = ResultWinCondition.SCISSORS_BEATS_PAPER;
            this.notify(MatchEvents.PLAYER_A_WINS_ROUND);
        } else if (this.playerB.currentHand === Hand.ROCK && this.playerA.currentHand === Hand.SCISSORS) {
            // p-a scissors < p-b rock
            this.score[this.playerB.id] += 1;
            this.howWasRoundWon = ResultWinCondition.ROCK_BEATS_SCISSORS;
            this.notify(MatchEvents.PLAYER_B_WINS_ROUND);
        } else if (this.playerB.currentHand === Hand.ROCK && this.playerA.currentHand === Hand.PAPER) {
            // p-a paper > p-b rock
            this.score[this.playerA.id] += 1;
            this.howWasRoundWon = ResultWinCondition.PAPER_BEATS_ROCK;
            this.notify(MatchEvents.PLAYER_A_WINS_ROUND);
        } else if (this.playerB.currentHand === Hand.SCISSORS && this.playerA.currentHand === Hand.PAPER) {
            // p-a paper < p-b scissors
            this.score[this.playerB.id] += 1;
            this.howWasRoundWon = ResultWinCondition.SCISSORS_BEATS_PAPER;
            this.notify(MatchEvents.PLAYER_B_WINS_ROUND);
        } else {
            // tie
            this.howWasRoundWon = null;
