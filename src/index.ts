...
class Match extends Observable<Match, MatchEvents> {
  ...
  public getWinnerOfRound() {
    ...
    if (
      this.playerA.currentHand === Hand.ROCK &&
      this.playerB.currentHand === Hand.SCISSORS
    ) {
      ...
      this.playerA.currentHand === Hand.ROCK &&
      this.playerB.currentHand === Hand.PAPER
    ) {
      // p-a rock < p-b paper
      ...
      this.playerA.currentHand === Hand.SCISSORS &&
      this.playerB.currentHand === Hand.PAPER
    ) {
      ...
      this.playerB.currentHand === Hand.ROCK &&
      this.playerA.currentHand === Hand.SCISSORS
    ) {
      ...
      this.playerB.currentHand === Hand.ROCK &&
      this.playerA.currentHand === Hand.PAPER
    ) {
      // p-a paper > p-b rock
      this.score[this.playerA.id] += 1;
      this.howWasRoundWon = ResultWinCondition.PAPER_BEATS_ROCK;
    ...
    if (
      this.score[this.playerA.id] === 3 ||
      this.score[this.playerB.id] === 3
    ) {
  ...
  private getWinnerOfGame() {
    ...
    const winningPlayer: Player =
      this.score[this.playerA.id] === 3 ? this.playerA : this.playerB;
